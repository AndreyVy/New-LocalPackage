function main {
    ...
    #region create temp location to download exe installer
    $ParentDir = "$PsScriptRoot\output"
    if (Test-Path $ParentDir) {}
    else { $null = New-Item $ParentDir -ItemType Directory }
    #endregion create temp location to download exe installer

    #region download the latest msi installation
    $FileName = ((Invoke-WebRequest -Uri $url).Links | Where-Object {$_.href -like "vlc-*-win64.msi"}).href
    (New-Object System.Net.WebClient).DownloadFile("$url/$FileName", "$ParentDir\$FileName")
    #endregion download the latest msi installation

    # create vlcrc file with encoding: UTF-8 with BOM; Unix (LF)
    ( $vlcrc -replace "`r`n", "`n" ) | Set-Content -Path "$ParentDir\vlcrc" -NoNewline

    # get product info
    $MSiInfo = get-msiproperty -FilePath "$ParentDir\$FileName" | Select-Object ProductName, ProductVersion
    $AppName = $MSiInfo.ProductName
    $AppVersion = $MSiInfo.ProductVersion
    ...
    $ScriptTemplate = $ScriptTemplate -replace "<MsiName>", $FileName
...
function get-msiproperty {
    ...
    $READONLY = 0
    $msidb = (New-Object -ComObject WindowsInstaller.Installer ).OpenDatabase("$FilePath", $READONLY)
...
$PkgName = 'VideoLAN-VLC-<AppVersion>'
...
#region // PUT REQUIRED CODE FOR INSTALLATION BELOW THIS LINE // ****************************************************
    ...
    if (-not (Get-InstalledSoftware -DisplayName "$ProductName" -DisplayVersion "$ProductVersion")) {
        Start-WindowsInstaller -MSIFile "$PSScriptRoot\<MsiName>" -InstallParams "ADDLOCAL=ALL ALLUSERS=1 REBOOT=ReallySuppress /qn /L*v `"$env:Windir\Logs\$($PkgName)_appinstaller.log`""
    } else { Write-InstallLog "UserOutput: $PkgName is already installed" }

    Copy-PackageFile -copySource "$PSScriptRoot\vlcrc" -copyDestination "${env:ProgramFiles}\VideoLAN\VLC\" -verbose
...
function Copy-PackageFile {
    ...
    if (Test-Path -Path $copySource) {
        if (-not (Test-Path $copyDestination)) { New-Item $copyDestination -ItemType Directory -Force }
...
function Start-WindowsInstaller {
    ...
    if ((0,3010) -notcontains $exitCode ) { Write-InstallLog "Installation failed with exit code: $exitCode" -isError }
...
# Encoder (string)
#sout-avi-encoder=VLC Media Player - 3.0.4 Vetinari
...
# Output back-end (string)
#mmdevice-backend=any

# HDMI/SPDIF audio passthrough (integer)
