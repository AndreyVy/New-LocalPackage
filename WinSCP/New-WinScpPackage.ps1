function main {
    $LAST_ARRAY_ELEMENT=-1
    $url = "https://winscp.net"
    
    # get download page and file name and version
    $webpath = ((Invoke-WebRequest -Uri "$url/eng/download.php" -UseBasicParsing).links | Where-Object href -like '*download*.exe').href
    $FileName = ($webpath.Split('/')[$LAST_ARRAY_ELEMENT])
    $AppVersion = $FileName -split "-" | Sort-Object | Select-Object -First 1 

    # get direct download link
    $url = ((Invoke-WebRequest -Uri "$url/$webpath" -UseBasicParsing).links | Where-Object outerHTML -like "*Direct download*").href
...
$PkgName = 'WinSCP-<AppVersion>'

function Start-ProductInstall {
    [CmdletBinding()]
    param()
#region // PUT REQUIRED CODE FOR INSTALLATION BELOW THIS LINE // ****************************************************
    $ProductName = "WinSCP <AppVersion>"
    $ProductVersion = "<AppVersion>"
    if (-not (Get-InstalledSoftware -DisplayName "$ProductName" -DisplayVersion "$ProductVersion")) {
        Start-ExeInstaller -setupExeFile "$PSScriptRoot\<FileName>" -exeParams "/ALLUSERS /VERYSILENT /NORESTART /SUPPRESSMSGBOXES /LOG=`"$env:Windir\Logs\$($PkgName)_appinstaller.log`""
